package org.eth.lectures.week3;
...
public class IOUtilsExtractActivities {
    public static void main(String[] args) throws IOException {
    	...
    	Config config = ConfigUtils.createConfig(); // creates empty config
    	
    	//First create Scenario
        Scenario scenario = ScenarioUtils.createMutableScenario(config); // creates empty scenario
        Population population = scenario.getPopulation(); 

      //Read the population file of interest into the population container
        PopulationReader popReader = new PopulationReader(scenario); // we need to pass the scenario to the reader
      ...
      //To get the information we need, first have to go through all the persons in the population file and
        for (Person person: population.getPersons().values() ){ // always make sure that <id> is checked, to know what you are accessing
